...
class MLP:
    ...
    def fit(self, X, y, max_epochs, shuffle_data):
        ...
        # Now weights[] contains weights array for input layer connect with hidden layer
        ...
        # Actually this part can be ignored in this special input arribute
        ...
        # weights[-1] now contains weight array for hidden layer connect with output layer
        ...
        # for input layer
        # self.batch_size rows and self.input_size columns，I think this represents self.batch_size * n_feature input information
        self.layers.append(np.empty((self.batch_size, self.input_size)))
        # for hidden layers
        # In this example layers[1] represents the activation function value after processing by hidden layer
        ...
        # for output layer
        # In this example layers[2] represents the activation function value after processing by output layer
        # deltas[1] represents layers[2] - y
        ...
        for i in xrange(max_epochs):
            ...
            y = y[shuffle_order]
            # iterate every batch
            # pulling out self.batch_size samples each time
            for batch in xrange(0, n_samples, self.batch_size):
                #TODO call forward function
                forward(X[batch : batch + self.batch_size, : ])
                #TODO call backward function
                backward(X[batch : batch + self.batch_size, : ], y[batch : batch + self.batch_size])
    ...
    def backward(self, X, y):
        ...
        dW2 = self.deltas[-1] * self.activation_func(self.layers[-2])
        dW1 = self.deltas[-2] * self.activation_func(self.layers[0])
